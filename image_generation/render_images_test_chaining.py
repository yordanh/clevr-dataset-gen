...
parser.add_argument('--properties_json', default='data/properties_test.json',
...
parser.add_argument('--min_dist', default=0.6, type=float,
...
def calculate_trajectory(init_position, intermediate_positions, args, curvy=False):
  traj = []

  segment_steps = (args.num_images-1) / len(intermediate_positions)
  intermediate_positions = [init_position] + intermediate_positions

  for i in range(1, len(intermediate_positions)):
    start = intermediate_positions[i-1]
    end = intermediate_positions[i]
    if not curvy:
      a = np.linspace(start[0], end[0], segment_steps+1, endpoint=True)[:, None]
      b = np.linspace(start[1], end[1], segment_steps+1, endpoint=True)[:, None]
      c = np.linspace(start[2], end[2], segment_steps+1, endpoint=True)[:, None]

      positions = np.concatenate((a[1:],b[1:],c[1:]), axis=1)
    else:
      points = [start, [0, 0, 5], end]
      positions = Bezier(points, num=segment_steps+1)[1:]

    traj += [(1, position) for position in positions]
