...
def calculate_trajectory(init_positions, final_positions, args, curvy=False):
  traj = []

  segment_steps = (args.num_images-1) / (args.max_objects - 1)
  for obj_idx in range(1, args.max_objects):
    start = init_positions[obj_idx]
    end = final_positions[obj_idx]
    if not curvy:
    ...
    traj += [(obj_idx, position) for position in positions]
...
def Bernstein(n, k):
    ...
    if k == 0:
      ...
      print("USE SCIPY BINOM FUNCTION.")
...
def check_visibility(blender_objects, objects, min_pixels_per_object):
  ...
  for i, obj in enumerate(objects):
    ...
    pixel_coords = [255 if x > 255 else x for x in pixel_coords]
